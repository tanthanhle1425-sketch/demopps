...
# --- Cấu hình giao diện ---
st.set_page_config(page_title="Tiểu luận Nhóm 06 - Đề tài 06", layout="wide")

# --- Phần 1: Tiêu đề và Mục tiêu ---
st.title(" Sản phẩm Demo - Đề tài 06")
st.markdown("""
**Đề tài:** Sự lan truyền sai số trong tính toán số và ảnh hưởng của nhiễu dữ liệu (Noise) đến mô hình xấp xỉ hàm.
""")

# --- Sidebar: Các thanh trượt điều khiển ---
st.sidebar.header("Cấu hình thực nghiệm")
st.sidebar.markdown("Thay đổi các thông số dưới đây để thấy sự lan truyền sai số: ")
...
with col1:
    st.subheader("Đồ thị mô phỏng sự lan truyền sai số ")
...
st.divider()
expander = st.expander("Xem giải thích 3 câu hỏi định hướng (Phần 2 - Bước 4)")
with expander:
    st.markdown("""
    1. **Tại sao đường Lagrange dao động biên độ lớn?** 
       - Do nội suy Lagrange bắt buộc phải đi qua chính xác các điểm nhiễu. Với đa thức bậc cao (n=14), các sai số nhỏ tại điểm nút bị phóng đại ở khoảng giữa các điểm, gây ra hiện tượng dao động mạnh (Runge's phenomenon).
    
    2. **Đường bình phương bé nhất có đi qua các điểm nhiễu không?**  
       - Không nhất thiết. Nó tìm cách tối thiểu hóa tổng bình phương khoảng cách từ các điểm đến đường cong, tạo ra một đường "trung bình" mịn hơn, do đó ít bị ảnh hưởng bởi các điểm nhiễu đơn lẻ. 
    
    3. **Nên chọn phương pháp nào khi dữ liệu chứa nhiễu?** 
       - Ta nên chọn **Phương pháp Bình phương bé nhất** để mô hình hóa, vì nó có khả năng lọc nhiễu và phản ánh xu hướng thực tế của hàm số tốt hơn là cố gắng khớp mọi điểm sai số. 
    """)

st.caption("Ứng dụng được xây dựng theo tài liệu hướng dẫn Đề tài 06 - Python cho Khoa học dữ liệu. ")
